  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          {/* Main Stats Card */}
          <div className="md:col-span-8 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[220px]">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
              <div className="absolute bottom-0 left-0 w-40 h-40 bg-fuchsia-500/30 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3"></div>
              ...
              <div className="relative z-10 mt-6 grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-black/20 backdrop-blur-sm rounded-2xl p-3 border border-white/10">
                      ...
                      <div className="text-xl font-bold text-emerald-300 flex items-center gap-1">
                          {purchasedItems.length} <Check className="w-4 h-4" />
                      </div>
                  </div>
                  <div className="bg-black/20 backdrop-blur-sm rounded-2xl p-3 border border-white/10">
                      <div className="text-[10px] text-indigo-200 font-bold uppercase mb-1">Total Terbeli</div>
                      <div className="text-sm font-bold truncate" title={formatCurrency(totalSpent)}>
          ...
          <button 
              ...
              className="md:col-span-4 bg-white rounded-[2.5rem] p-6 border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-200 hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col items-center justify-center gap-4 text-center cursor-pointer"
          >
              ...
              <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center group-hover:bg-indigo-600 transition-colors z-10 shadow-inner">
                  <Plus className="w-8 h-8 text-slate-400 group-hover:text-white transition-colors" />
              </div>
              <div className="z-10">
                  <h3 className="text-lg font-bold text-slate-800">Tambah Wishlist</h3>
                  <p className="text-xs text-slate-400 mt-1">Untuk periode {monthLabel}</p>
              </div>
          </button>
      </div>
      ...
      {isFormOpen && (
          <div className="relative z-20 animate-in fade-in slide-in-from-top-4">
      ...
      <div className="space-y-4">
          ...
          {filteredItems.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-[2rem] border border-dashed border-slate-200">
                  ...
                  <p className="text-slate-500 font-medium">Tidak ada target belanja.</p>
                  ...
                  {sortedItems.map((item) => {
                      ...
                      return (
                          <div 
                              key={item.id} 
                              className={`
                                  relative bg-white rounded-[1.5rem] p-5 border transition-all duration-300 group
                                  ${isPurchased 
                          ...
                          >
                              ...
                              <div>
                                  <h4 className={`font-bold text-lg mb-1 ${isPurchased ? 'text-slate-500 line-through' : 'text-slate-800'}`}>
                                      {item.itemName}
                                  </h4>
                                  <div className={`text-2xl font-black tracking-tight ${isPurchased ? 'text-slate-400' : 'text-indigo-600'}`}>
